...
class ModelManager:
    ...
    def __init__(self):
        ...
        self.model_save_path = self.management_config.get('model_save_path', './models/saved/')
    ...
    def train_model(self, df: pd.DataFrame, retrain: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # Split data
            train_size = int(len(X_scaled) * (1 - self.config.get('validation_split', 0.2)))
    ...
    def _save_metadata(self, timestamp: str) -> bool:
        """Save model metadata."""
        try:
            metadata_path = os.path.join(self.model_save_path, f"metadata_{timestamp}.pkl")
    ...
    def should_retrain(self) -> bool:
        ...
        try:
            ...
            # Parse timestamp
            last_training_dt = datetime.fromisoformat(last_training.replace('Z', '+00:00'))
        ...
        try:
            ...
            for filepath, _ in model_files[keep_count:]:
                try:
                    ...
                    fe_file = os.path.join(self.model_save_path, f"feature_engineer_{timestamp}.pkl")
                    if os.path.exists(fe_file):
                        os.remove(fe_file)
                    
                    # Remove metadata file
                    metadata_file = os.path.join(self.model_save_path, f"metadata_{timestamp}.pkl")
