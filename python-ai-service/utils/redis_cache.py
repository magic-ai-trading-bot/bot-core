...
class CacheManager:
    ...
    async def cache_technical_indicators(self, symbol: str, timeframe: str, 
                                       indicators: dict, ttl: int = 180):
        """Cache technical indicators"""
        key = f"indicators:{symbol}:{timeframe}"
        await self._cache.set(key, indicators, ttl)
    
    async def get_technical_indicators(self, symbol: str, timeframe: str) -> Optional[dict]:
