...
class FeatureEngineer:
    ...
    def _create_target(self, df: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            target = np.where(future_return > 0.005, 1.0,  # Strong buy signal
                             np.where(future_return < -0.005, 0.0,  # Strong sell signal
                                     0.5 + (future_return * 50)))  # Neutral to weak signals
    ...
    def prepare_for_inference(self, df: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            if len(processed_df) < sequence_length:
                logger.warning(f"Not enough data for inference. Required: {sequence_length}, Available: {len(processed_df)}")
                return None
            
            # Select feature columns
            if not self.feature_columns:
                # If feature columns not set, use all except target columns
                feature_cols = [col for col in processed_df.columns if col not in ['target', 'signal']]
            else:
