...
class TechnicalIndicators:
    ...
    def calculate_ema(self, df: pd.DataFrame, periods: Optional[list] = None) -> Dict[str, pd.Series]:
    ...
    def detect_price_patterns(self, df: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        try:
            ...
            window = 20
            patterns['local_high'] = df['high'].rolling(window=window, center=True).max() == df['high']
            patterns['local_low'] = df['low'].rolling(window=window, center=True).min() == df['low']
            
            # Price breakouts
            patterns['breakout_high'] = df['close'] > df['high'].rolling(window=20).max().shift(1)
            patterns['breakout_low'] = df['close'] < df['low'].rolling(window=20).min().shift(1)
    ...
    def calculate_all_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Successfully calculated indicators. DataFrame shape: {enriched_df.shape}")
            return enriched_df
