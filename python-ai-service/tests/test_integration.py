...
class TestFullAnalysisFlow:
    ...
    async def test_periodic_analysis_task(self, mock_openai_client, mock_mongodb):
        ...
        mock_candles = [[1701234567000, "45000", "45500", "44800", "45200", "1000", 
...
class TestAPIKeyRotation:
    ...
    async def test_api_key_rotation_on_rate_limit(self, client, sample_ai_analysis_request):
        ...
        success_response.json.return_value = {
            "choices": [{
                "message": {
                    "content": '{"signal": "Short", "confidence": 0.78}'
                }
            }]
        }
        ...
        mock_client.post.side_effect = [
            httpx.HTTPStatusError("Rate limited", request=MagicMock(), response=rate_limit_response),
            httpx.HTTPStatusError("Rate limited", request=MagicMock(), response=rate_limit_response),
            success_response
        ]
        
        with patch('httpx.AsyncClient', return_value=mock_client):
            result = await call_openai_with_fallback(api_keys, [{"role": "user", "content": "test"}])
