...
class TestConfig:
    ...
    def test_load_config_from_yaml(self, mock_open, mock_exists):
        ...
        mock_open.return_value.__enter__.return_value.read.return_value = yaml.dump(mock_config_data)
    ...
    def test_default_config_structure(self, mock_exists):
        ...
        required_sections = [
            "server", "model", "trading", "technical_indicators",
            "data", "ai_cache", "model_management", "logging"
        ]
    ...
    @patch('config.config.Path.exists', return_value=False)
    @patch.dict(os.environ, {
        "SERVER_PORT": "invalid_port"  # Invalid port number
    })
