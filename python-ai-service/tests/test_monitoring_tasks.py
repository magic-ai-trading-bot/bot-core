...
class TestSystemHealthCheck:
    ...
    def test_health_check_all_services_healthy(self, mock_get):
        ...
        assert result['health_report']['overall_status'] in ['healthy', 'degraded', 'critical']
...
class TestDailyAPICostReport:
    ...
    def test_api_cost_report_no_data(self, mock_get):
        ...
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'session_statistics': {'total_requests': 0, 'total_cost_usd': 0, 'total_cost_vnd': 0},
            'projections': {'estimated_daily_cost_usd': 0, 'estimated_monthly_cost_usd': 0}
        }
        mock_get.return_value = mock_response
...
class TestDailyPerformanceAnalysis:
    ...
    def test_performance_analysis_trend_detection(self, mock_storage):
        ...
        mock_storage.get_performance_metrics_history.return_value = [
            ...
            {'date': (datetime.now() - timedelta(days=1)).date(), 'win_rate': 55.0, 'avg_profit': 1.0},
            {'date': (datetime.now() - timedelta(days=2)).date(), 'win_rate': 65.0, 'avg_profit': 1.5},
            {'date': (datetime.now() - timedelta(days=3)).date(), 'win_rate': 70.0, 'avg_profit': 2.0}
        ]
