...
def main():
    ...
    response = requests.get(
        f"{RUST_API_URL}/api/paper-trading/trades/closed", timeout=10
    )
        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional crypto trading analyst. Analyze trades and provide actionable feedback. Always respond in valid JSON format.",
                    },
            ...
            result = storage.store_trade_analysis(
                trade_id=trade_id, trade_data=trade, analysis=analysis
            )

            print(f"  ✅ {symbol} analyzed and saved! (ID: {result})")
            print(
                f'     Assessment: {analysis.get("overall_assessment", "N/A")[:100]}...'
            )
