...
class TradingServiceClient:
    ...
    def train_model(self, candles: List[Dict], model_type: str = "lstm", retrain: bool = False) -> Dict[str, Any]:
    ...
    def analyze_market(self, candles: List[Dict], symbol: str = SYMBOL, timeframe: str = TIMEFRAME) -> Dict[str, Any]:
...
def generate_sample_candles(count: int = 500, start_price: float = 47000.0) -> List[Dict[str, Any]]:
...
def main():
    ...
    if health:
        ...
        print("❌ Service unavailable. Make sure the service is running on localhost:8000")
        return
    ...
    if config:
        supported_timeframes = config.get('data_config', {}).get('supported_timeframes', [])
    ...
    print(f"   Latest candle: Open={latest_candle['open']}, Close={latest_candle['close']}, Volume={latest_candle['volume']}")
    ...
    if not model_loaded:
        ...
        if training_result:
            ...
            for i in range(10):
                ...
                print("⚠️  Training is taking longer than expected. Proceeding with analysis attempt...")
        else:
    ...
    if final_model_info:
        ...
        print(f"   Validation samples: {final_model_info.get('validation_samples', 'N/A')}")
        print(f"   Features: {final_model_info.get('feature_count', 'N/A')}")
        print(f"   Training accuracy: {final_model_info.get('training_accuracy', 'N/A')}")
