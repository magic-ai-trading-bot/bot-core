...
4. **Scalability** - Sharding-ready design for horizontal scaling
...
├── trade_records                  # Live trading records (real trading)
├── market_data                    # OHLCV candlestick data
├── price_history                  # Price history time-series
├── ai_analysis_results            # AI-generated analysis (Python service)
├── analysis_results               # AI analysis cache (Rust service)
├── ai_signals                     # AI signal execution log
├── signals_history                # Historical signals
├── paper_trades                   # Paper trading simulated trades
├── paper_trading_settings         # Paper trading configurations
├── portfolio_history              # Paper trading portfolio snapshots
├── performance_metrics            # Daily performance aggregations
├── api_keys                       # User API keys for exchanges
├── user_symbols                   # User-configured trading symbols
├── notification_preferences       # User notification settings
├── push_subscriptions             # Web push subscription endpoints
├── trade_analyses                 # Per-trade AI analysis results
└── config_suggestions             # AI-generated config suggestions
```
...
- **Authentication:** 2 collections (users, sessions)
- **Trading:** 2 collections (trade_records, api_keys)
- **Paper Trading:** 3 collections (paper_trades, paper_trading_settings, portfolio_history)
- **Market Data:** 3 collections (market_data, price_history, signals_history)
- **AI/ML:** 5 collections (ai_analysis_results, analysis_results, ai_signals, trade_analyses, config_suggestions)
- **Performance:** 1 collection (performance_metrics)
- **User Config:** 3 collections (user_symbols, notification_preferences, push_subscriptions)
...
### 3. positions (IN-MEMORY ONLY - Not a MongoDB Collection)

**Purpose:** Currently open trading positions with real-time P&L

> **IMPORTANT:** Positions are tracked **in-memory** using a DashMap in the Rust Core Engine for sub-millisecond access. They are **NOT persisted to MongoDB**. On engine restart, position state is rebuilt from trade records. This section documents the in-memory data structure only.
...
### 6. portfolio_history Collection

**Purpose:** Historical portfolio value tracking for performance charts (paper trading snapshots)
...
**Indexes:**
```javascript
db.portfolio_history.createIndex({ "user_id": 1, "snapshot_time": -1 })
db.portfolio_history.createIndex({ "snapshot_time": -1 })
db.portfolio_history.createIndex({ "created_at": 1 }, { expireAfterSeconds: 7776000 })  // TTL 90 days
```
...
### 7. paper_trading_settings Collection (embedded account state)

**Purpose:** Paper trading configuration and persistent account settings

> **Note:** The paper trading account state (balances, positions, metrics) is maintained in-memory in the Rust engine. The `paper_trading_settings` collection persists user configuration. Portfolio state is captured periodically in `portfolio_history`.
...
**Indexes:**
```javascript
db.paper_trading_settings.createIndex({ "user_id": 1 }, { unique: true })
```
...
**Indexes:**
```javascript
db.paper_trades.createIndex({ "trade_id": 1 }, { unique: true })
db.paper_trades.createIndex({ "user_id": 1, "created_at": -1 })
db.paper_trades.createIndex({ "symbol": 1, "created_at": -1 })
db.paper_trades.createIndex({ "status": 1 })
db.paper_trades.createIndex({ "ai_signal_id": 1 }, { sparse: true })
```
...
### 9. audit_logs Collection (NOT IMPLEMENTED - Planned)

> **Status:** This collection does not exist in the current codebase. It is a planned feature for a future security audit trail implementation.

**Purpose:** System-wide security and activity audit trail (planned)
...
### 10. system_config Collection (NOT IMPLEMENTED - Planned)

> **Status:** This collection does not exist in the current codebase. System configuration is managed via environment variables.

**Purpose:** Global system configuration and feature flags (planned)
...
### 15. notification_preferences Collection + push_subscriptions Collection
User notification settings and web push subscription endpoints.
- `notification_preferences` - per-user notification configuration
- `push_subscriptions` - web push API subscription data

### 16. sessions Collection
...
**Purpose:** Store Celery task execution metadata and results (managed by Celery) — PLANNED, not implemented
...
### 19. training_jobs Collection (NOT IMPLEMENTED)
...
### 20. backtest_results Collection (NOT IMPLEMENTED)
...
### 21. monitoring_alerts Collection (NOT IMPLEMENTED)
...
### 22. task_schedules Collection (NOT IMPLEMENTED)
...
```mermaid
erDiagram
    users ||--o{ trade_records : "places"
    users ||--o{ paper_trades : "simulates"
    users ||--o{ portfolio_history : "tracks"
    users ||--o{ paper_trading_settings : "configures"
    users ||--o{ user_symbols : "configures"
    users ||--o{ api_keys : "stores"
    users ||--o{ notification_preferences : "sets"
    users ||--o{ push_subscriptions : "registers"
    users ||--o{ sessions : "authenticates"

    paper_trades }o--|| ai_signals : "based on"
    trade_records }o--|| ai_analysis_results : "based on"

    market_data ||--o{ ai_analysis_results : "analyzed by"
    paper_trades ||--o{ trade_analyses : "analyzed by"
    paper_trades ||--o{ config_suggestions : "suggests"
```
...
| sessions | 7 days | TTL index | ~50MB |
| trade_records | 1 year | TTL index | ~2GB |
| market_data | 90 days | TTL index + time-series | ~6.2GB |
| price_history | 90 days | TTL index | ~1GB |
| ai_analysis_results | 30 days | TTL index | ~900MB |
| analysis_results | 30 days | TTL index | ~500MB |
| portfolio_history | 90 days | TTL index | ~5GB |
| paper_trades | Permanent | User-controlled reset | ~2GB/year |
| paper_trading_settings | Permanent | User-controlled | ~10MB |
| ai_signals | 30 days | TTL index | ~500MB |
| signals_history | 90 days | TTL index | ~1GB |
| performance_metrics | 1 year | TTL index | ~200MB |
| trade_analyses | 90 days | TTL index | ~300MB |
| config_suggestions | 30 days | TTL index | ~50MB |
| notification_preferences | Permanent | Manual deletion only | <10MB |
| push_subscriptions | Permanent | User-controlled | <10MB |
| api_keys | Permanent | User-controlled | <10MB |
| user_symbols | Permanent | User-controlled | <10MB |

**Total Estimated Storage:** ~20GB (with retention policies applied)
