...
graph TB
    ...
    subgraph "Application Layer"
        subgraph "Rust Core Engine - Port 8080"
            RUST[Rust Trading Engine<br/>Warp + Tokio<br/>JWT Authentication]
        ...
        subgraph "MCP Server - Port 8090"
            MCP[MCP Server<br/>Node.js + Express<br/>110 Tools]
        end

        subgraph "OpenClaw Gateway - Port 18789"
            OCG[OpenClaw Gateway<br/>Node.js + WebSocket<br/>Telegram Integration]
        end
    end

    subgraph "Data Layer"
        DB[(MongoDB 7.0<br/>Local Container)]
        CACHE[Redis Cache<br/>Required for VPS]
    end

    subgraph "External Services"
        BINANCE[Binance API<br/>WebSocket + REST]
        GROK[xAI Grok API<br/>AI Analysis]
    end
...
- AI agent orchestration via Claude
...
graph LR
    ...
    subgraph "Backend Stack - Rust"
        RUST[Rust 1.86+]
        WARP_FW[Warp 0.4<br/>Web Framework]
        TOKIO[Tokio 1.49<br/>Async Runtime]
        SERDE[Serde 1.0<br/>Serialization]
        MONGODB_RUST[MongoDB Driver 3.5]
...
| Warp | 0.4 | Web Framework + WebSocket | `src/api/mod.rs`, `src/binance/websocket.rs` |
| Tokio | 1.49 | Async Runtime | Throughout |
| Serde | 1.0 | Serialization | Data models |
| MongoDB | 3.5 | Database Driver | `src/storage/` |
| Reqwest | 0.12 | HTTP Client | `src/binance/client.rs` |
| JWT | 10.3 | Authentication | `src/auth/jwt.rs` |
| BCrypt | 0.17 | Password Hashing | `src/auth/handlers.rs` |
...
|------------|---------|---------|---------------|
| MongoDB | 7.0 | Primary Database (local container) | Connection string in `.env` |
| Redis | 7-alpine | Cache (required for VPS deployment) | Port 6379, Docker Compose |
...
- **Note**: OpenAI SDK is used for compatibility (base_url overridden to api.x.ai)
...
- **Python → xAI Grok**: API key (Bearer token)
...
| Python → xAI Grok | HTTPS | 1000ms | 5000ms | 20 req/min |
...
| AI Requests/Min | 5 | 10 | 20 (xAI Grok limit) |
...
| xAI Grok API failures | Medium | High | Multiple API keys, fallback to technical analysis |
