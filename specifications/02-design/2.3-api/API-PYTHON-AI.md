...
The Python AI Service provides xAI Grok powered trading signal generation, technical analysis, strategy recommendations, and market condition analysis. It integrates with the xAI Grok API (via OpenAI-compatible SDK) for intelligent trading decisions.

**Service Architecture:**
- **Language:** Python 3.11+
- **Framework:** FastAPI
- **AI Model:** `grok-4-1-fast-non-reasoning` (xAI Grok, via OpenAI-compatible SDK with base_url=api.x.ai)
...
**Key Features:**
- xAI Grok powered signal generation
...
**Description:** Debug xAI Grok connectivity and API key status
...
```json
{
  "service": "Grok AI Cryptocurrency Trading Service",
  "version": "2.0.0",
  "description": "Advanced AI-powered trading signal generation using xAI Grok with MongoDB storage and real-time WebSocket broadcasting",
  "endpoints": {
    "analyze": "POST /ai/analyze - Generate trading signals with Grok AI (stored in MongoDB)",
...
**Description:** Analyze trading signals using xAI Grok with MongoDB caching
...
```json
{
  "signal": "Long",
  "confidence": 0.82,
  "reasoning": "Strong bullish momentum with RSI oversold (28), MACD crossover, and positive Grok AI sentiment analysis. Multiple timeframes confirm uptrend.",
  "strategy_scores": {
    "RSI Strategy": 0.85,
    "MACD Strategy": 0.78,
...
```json
{
  "detail": "AI analysis failed: xAI Grok rate limit exceeded"
}
```
...
```json
{
  "service_name": "Grok AI Cryptocurrency Trading Service",
  "version": "2.0.0",
...
## Async Task Management (NOT IMPLEMENTED - Planned)

> **Status:** The following endpoints in this section do NOT exist in the current codebase. The Python service does NOT use Celery or RabbitMQ. Task files exist in `python-ai-service/tasks/` but are not exposed as API endpoints. These endpoints are planned for a future async execution system.

### POST /api/tasks/train (NOT IMPLEMENTED)

**Description:** Trigger async model training job — PLANNED, not implemented
...
## Training Management (NOT IMPLEMENTED - Planned)

> **Status:** These endpoints do NOT exist in the current codebase.

### GET /api/training/jobs (NOT IMPLEMENTED)
...
## Backtest Management (NOT IMPLEMENTED - Planned)

> **Status:** These endpoints do NOT exist in the current codebase.

### POST /api/backtests (NOT IMPLEMENTED)
...
## Monitoring & Alerts (NOT IMPLEMENTED - Planned)

> **Status:** These endpoints do NOT exist in the current codebase.

### GET /api/monitoring/health (NOT IMPLEMENTED)

**Description:** Get comprehensive system health status including all services — PLANNED, not implemented
...
|-------|-------------|----------|
| `AI analysis failed: <reason>` | Analysis request failed | Check request format and xAI Grok status |
| `xAI Grok rate limit exceeded` | Too many xAI API calls | Wait for rate limit reset or use backup keys |
| `MongoDB not connected` | Database unavailable | Check MongoDB connection |
| `Grok client not initialized` | Grok client setup failed | Verify XAI_API_KEY configuration |
...
### xAI Grok Rate Limiting

**grok-4-1-fast-non-reasoning Rate Limits:**
...
| Grok Analyzer | `python-ai-service/main.py:1071-1535` | xAI Grok trading analysis engine |
...
| GrokClient | `python-ai-service/main.py:903-1066` | HTTP-based xAI Grok client (OpenAI-compatible) |
...
**Description:** Get xAI Grok API cost statistics and usage tracking
...
```json
{
  ...
  "requests_by_model": {
    "grok-4-1-fast-non-reasoning": 1250
  },
  "requests_today": 48,
  "cost_today_usd": 0.48,
...
**Description:** View historical xAI Grok analysis results with pagination
