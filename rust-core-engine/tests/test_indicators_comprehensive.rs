...
fn test_rsi_flat_prices_equals_50() {
    ...
    if let Some(&last_rsi) = rsi.last() {
        ...
        if !last_rsi.is_nan() {
            assert!((last_rsi - 50.0).abs() < 1.0,
                "RSI for flat prices should be near 50.0, got {}", last_rsi);
        }
    }
}
...
fn test_rsi_minimum_data_points() {
    ...
    assert!(rsi.is_ok(), "Should work with exactly 15 data points for RSI-14");

    // Verify the result has the correct length and valid values
    let rsi_values = rsi.unwrap();
    assert_eq!(rsi_values.len(), 1, "Should have exactly 1 RSI value with 15 data points");
    let last_rsi = *rsi_values.last().unwrap();
    assert!(last_rsi >= 0.0 && last_rsi <= 100.0, "RSI must be in range [0, 100], got {}", last_rsi);
    // With constantly increasing prices, RSI should be very high
    assert!(last_rsi > 95.0, "Constantly increasing prices should give RSI > 95, got {}", last_rsi);
}
...
fn test_rsi_alternating_prices() {
    ...
    let prices: Vec<f64> = (0..30).map(|i| if i % 2 == 0 { 100.0 } else { 101.0 }).collect();
    let candles = create_candles(prices);

    let rsi = calculate_rsi(&candles, 14).unwrap();
    let last_rsi = *rsi.last().unwrap();

    assert!(last_rsi > 40.0 && last_rsi < 60.0, "Alternating prices should give neutral RSI: {}", last_rsi);
}
...
fn test_rsi_all_gains() {
    ...
    assert!(last_rsi > 98.0, "All gains should give RSI near 100: {}", last_rsi);
}
...
fn test_rsi_different_periods() {
    ...
    assert!(rsi_7.len() > rsi_21.len(), "Shorter period should have more values");
}
...
fn test_macd_bullish_crossover() {
    ...
    assert!(!macd.signal_line.is_empty(), "Signal line should have values");
    assert!(!macd.histogram.is_empty(), "Histogram should have values");
    assert_eq!(macd.histogram.len(), macd.signal_line.len(), "Histogram and signal should match");
    ...
    assert!(last_macd > 0.0, "MACD line should be positive in uptrend, got {}", last_macd);
...
fn test_macd_bearish_crossover() {
    ...
    assert!(last_macd < 0.0, "MACD line should be negative in downtrend, got {}", last_macd);
...
fn test_macd_flat_market() {
    ...
    assert!((last_macd).abs() < 0.01, "Flat market should have MACD near 0");
    assert!((last_signal).abs() < 0.01, "Flat market should have signal near 0");
    assert!((last_histogram).abs() < 0.01, "Flat market should have histogram near 0");
}
...
fn test_macd_custom_parameters() {
    ...
    assert_eq!(macd.histogram.len(), macd.signal_line.len(), "Histogram and signal should have same length");
}
...
fn test_macd_histogram_calculation() {
    ...
    if !macd.histogram.is_empty() && !macd.signal_line.is_empty() {
        ...
        for (i, &hist) in macd.histogram.iter().enumerate() {
            ...
            assert!((hist - expected).abs() < 0.0001, "Histogram calculation mismatch at {}: {} vs {}", i, hist, expected);
        }
    }
}
...
fn test_bollinger_bands_relationship() {
    ...
    for i in 0..bb.upper.len() {
        assert!(bb.upper[i] > bb.middle[i], "Upper band should be > middle at {}", i);
        assert!(bb.middle[i] > bb.lower[i], "Middle band should be > lower at {}", i);
    }
}
...
fn test_bollinger_bands_flat_prices() {
    ...
    assert!((last_middle - 100.0).abs() < 0.01, "Middle band should be at price");
    assert!((last_upper - 100.0).abs() < 0.01, "Upper band should converge to price");
    assert!((last_lower - 100.0).abs() < 0.01, "Lower band should converge to price");
}
...
fn test_bollinger_bands_expansion() {
    ...
    assert!(bandwidth > 0.05, "High volatility should create wider bands");
}
...
fn test_volume_profile_poc_accuracy() {
    ...
    assert!((vp.poc - 100.0).abs() < 1.0, "POC should be near high volume price: {}", vp.poc);
}
...
fn test_ema_basic_calculation() {
    ...
    assert!(last_ema > 22.0 && last_ema < 26.0, "EMA should be within price range");
}

#[test]
fn test_ema_vs_sma_responsiveness() {
    let prices: Vec<f64> = (0..20).map(|i| if i < 10 { 100.0 } else { 110.0 }).collect();
...
fn test_ema_flat_prices() {
    ...
    for &val in &ema {
        assert!((val - 100.0).abs() < 0.01, "EMA of flat prices should equal the price");
    }
}
