...
async fn test_get_status_returns_valid_response() {
    ...
    match response {
        Ok(resp) => {
            ...
            assert!(!status.circuit_breaker_open, "Circuit breaker should be closed");
        }
        Err(e) => {
    ...
    match response {
        Ok(resp) => {
            if !resp.status().is_success() {
                println!("⚠️ Portfolio endpoint returned non-200, server may not be fully configured");
                return;
            }
            ...
            assert!(portfolio.total_balance >= 0.0, "Balance should be non-negative");
        }
        Err(e) => {
            if e.is_connect() {
                println!("⚠️ Server not running, skipping integration test");
...
async fn test_get_settings_returns_valid_config() {
    ...
    match response {
        Ok(resp) => {
            ...
            assert!(settings.max_leverage <= 10, "Max leverage should be capped at 10x");
            assert!(settings.max_position_size_usdt > 0.0, "Max position size should be positive");
        }
        Err(e) => {
            if e.is_connect() {
                println!("⚠️ Server not running, skipping integration test");
...
async fn test_place_order_returns_confirmation_token() {
    ...
    match response {
        Ok(resp) => {
            ...
            assert!(confirmation.requires_confirmation, "Should require confirmation");
            assert!(!confirmation.token.is_empty(), "Token should not be empty");
            assert!(confirmation.summary.contains("BTCUSDT"), "Summary should contain symbol");
        }
        Err(e) => {
            if e.is_connect() {
                println!("⚠️ Server not running, skipping integration test");
    ...
    match response {
        Ok(resp) => {
            ...
            if let Some(orders) = body.data {
                ...
                for order in &orders {
                    assert_eq!(order.symbol, "BTCUSDT", "Filtered orders should match symbol");
                }
            }
        }
        Err(e) => {
...
async fn test_cancel_nonexistent_order_returns_error() {
    let client = create_client();
    let url = format!("{}/api/real-trading/orders/nonexistent-id-12345", get_api_base());
...
async fn test_modify_sltp_nonexistent_position() {
    let client = create_client();
    let url = format!("{}/api/real-trading/positions/INVALIDUSDT/sltp", get_api_base());
    ...
    match response {
        Ok(resp) => {
            ...
            if let Some(status) = body.data {
                assert!(status.is_testnet, "CRITICAL: Production mode detected! Tests should only run on testnet.");
            }
        }
        Err(e) => {
            if e.is_connect() {
