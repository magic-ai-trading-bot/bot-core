...
impl VolumeStrategy {
    pub fn new() -> Self {
        ...
        config.parameters.insert("volume_sma_period".to_string(), json!(20));
        config.parameters.insert("volume_spike_threshold".to_string(), json!(2.0));
        config.parameters.insert("price_volume_correlation_period".to_string(), json!(10));
...
impl Strategy for VolumeStrategy {
    ...
    async fn analyze(&self, data: &StrategyInput) -> Result<StrategyOutput, StrategyError> {
        ...
        let candles = data.timeframe_data.get(primary_timeframe)
        ...
        if volume_sma.is_empty() {
            return Err(StrategyError::InsufficientData("No volume SMA calculated".to_string()));
        }
        ...
        let recent_price_changes = &price_changes[price_changes.len() - recent_period.min(price_changes.len())..];
...
impl VolumeStrategy {
    fn analyze_volume_signals(
        ...
        let bullish_volume_ratio = if total_volume > 0.0 { bullish_volume / total_volume } else { 0.5 };
