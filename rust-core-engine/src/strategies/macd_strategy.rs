...
impl MacdStrategy {
    pub fn new() -> Self {
        ...
        config.parameters.insert("fast_period".to_string(), json!(12));
        config.parameters.insert("slow_period".to_string(), json!(26));
        config.parameters.insert("signal_period".to_string(), json!(9));
        config.parameters.insert("histogram_threshold".to_string(), json!(0.001));
...
impl Strategy for MacdStrategy {
    ...
    async fn analyze(&self, data: &StrategyInput) -> Result<StrategyOutput, StrategyError> {
        ...
        let primary_candles = data.timeframe_data.get(primary_timeframe)
            .ok_or_else(|| StrategyError::InsufficientData(format!("Missing {} data", primary_timeframe)))?;
            
        let confirmation_candles = data.timeframe_data.get(confirmation_timeframe)
            .ok_or_else(|| StrategyError::InsufficientData(format!("Missing {} data", confirmation_timeframe)))?;
        ...
        let confirmation_macd = calculate_macd(confirmation_candles, fast_period, slow_period, signal_period)
            .map_err(|e| StrategyError::CalculationError(e))?;
        
        if primary_macd.histogram.is_empty() || confirmation_macd.histogram.is_empty() {
            return Err(StrategyError::InsufficientData("No MACD values calculated".to_string()));
        }
        ...
        let (signal, confidence, reasoning) = self.analyze_macd_signals(
            current_macd_1h, current_signal_1h, current_histogram_1h,
            current_macd_4h, current_signal_4h, current_histogram_4h,
            prev_macd_1h, prev_signal_1h, prev_histogram_1h, prev_histogram_4h,
        );
    ...
    fn validate_data(&self, data: &StrategyInput) -> Result<(), StrategyError> {
        ...
        for timeframe in required_timeframes {
            let candles = data.timeframe_data.get(timeframe)
                .ok_or_else(|| StrategyError::DataValidation(format!("Missing {} timeframe data", timeframe)))?;
            ...
            if candles.len() < min_required {
                return Err(StrategyError::InsufficientData(
                    format!("Need at least {} candles for {} timeframe, got {}", 
...
impl MacdStrategy {
    fn analyze_macd_signals(
        &self,
        macd_1h: f64, signal_1h: f64, histogram_1h: f64,
        macd_4h: f64, signal_4h: f64, histogram_4h: f64,
        prev_macd_1h: f64, prev_signal_1h: f64, prev_histogram_1h: f64, prev_histogram_4h: f64,
        ...
        if bullish_crossover_1h && histogram_above_zero_4h && histogram_increasing_1h && histogram_increasing_4h {
            return (
                TradingSignal::Long,
                0.89,
        ...
        if bearish_crossover_1h && histogram_below_zero_4h && histogram_decreasing_1h && histogram_decreasing_4h {
            return (
                TradingSignal::Short,
                0.89,
