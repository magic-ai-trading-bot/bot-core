...
impl BollingerStrategy {
    pub fn new() -> Self {
        ...
        config.parameters.insert("bb_multiplier".to_string(), json!(2.0));
        config.parameters.insert("squeeze_threshold".to_string(), json!(0.02));
...
impl Strategy for BollingerStrategy {
    ...
    async fn analyze(&self, data: &StrategyInput) -> Result<StrategyOutput, StrategyError> {
        ...
        let confirmation_bb = calculate_bollinger_bands(confirmation_candles, bb_period, bb_multiplier)
            .map_err(|e| StrategyError::CalculationError(e))?;
        
        if primary_bb.upper.is_empty() || confirmation_bb.upper.is_empty() {
            return Err(StrategyError::InsufficientData("No Bollinger Bands calculated".to_string()));
        }
        ...
        let (signal, confidence, reasoning) = self.analyze_bollinger_signals(
            current_price,
            upper_1h, middle_1h, lower_1h,
            upper_4h, middle_4h, lower_4h,
            bb_position_1h, bb_position_4h,
            bb_width_1h, bb_width_4h,
            is_squeeze_1h, is_squeeze_4h,
            bb_expanding_1h, bb_contracting_1h,
        );
...
impl BollingerStrategy {
    fn analyze_bollinger_signals(
        &self,
        current_price: f64,
        upper_1h: f64, middle_1h: f64, lower_1h: f64,
        upper_4h: f64, middle_4h: f64, lower_4h: f64,
        bb_position_1h: f64, bb_position_4h: f64,
        bb_width_1h: f64, bb_width_4h: f64,
        is_squeeze_1h: bool, is_squeeze_4h: bool,
        bb_expanding_1h: bool, bb_contracting_1h: bool,
