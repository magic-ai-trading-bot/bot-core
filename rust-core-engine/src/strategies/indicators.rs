...
pub fn calculate_rsi(candles: &[CandleData], period: usize) -> Result<Vec<f64>, String> {
    if candles.len() < period + 1 {
        return Err(format!("Need at least {} candles for RSI calculation", period + 1));
    }
...
pub fn calculate_macd(candles: &[CandleData], fast: usize, slow: usize, signal: usize) -> Result<MacdResult, String> {
...
pub fn calculate_bollinger_bands(candles: &[CandleData], period: usize, multiplier: f64) -> Result<BollingerBands, String> {
...
pub fn calculate_volume_profile(candles: &[CandleData], levels: usize) -> Result<VolumeProfile, String> {
    if candles.is_empty() {
...
pub fn calculate_stochastic(candles: &[CandleData], k_period: usize, d_period: usize) -> Result<StochasticResult, String> {
    ...
    Ok(StochasticResult { k_percent, d_percent })
} 