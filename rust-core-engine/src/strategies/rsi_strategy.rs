...
impl RsiStrategy {
    pub fn new() -> Self {
        ...
        config.parameters.insert("oversold_threshold".to_string(), json!(30.0));
        config.parameters.insert("overbought_threshold".to_string(), json!(70.0));
        config.parameters.insert("extreme_oversold".to_string(), json!(20.0));
        config.parameters.insert("extreme_overbought".to_string(), json!(80.0));
...
impl Strategy for RsiStrategy {
    ...
    async fn analyze(&self, data: &StrategyInput) -> Result<StrategyOutput, StrategyError> {
        ...
        if primary_rsi.is_empty() || confirmation_rsi.is_empty() {
            return Err(StrategyError::InsufficientData("No RSI values calculated".to_string()));
        }
        ...
        let prev_rsi_1h = if primary_rsi.len() > 1 { primary_rsi[primary_rsi.len() - 2] } else { current_rsi_1h };
        let prev_rsi_4h = if confirmation_rsi.len() > 1 { confirmation_rsi[confirmation_rsi.len() - 2] } else { current_rsi_4h };
        ...
        let (signal, confidence, reasoning) = self.analyze_rsi_signals(
            current_rsi_1h, current_rsi_4h,
            prev_rsi_1h, prev_rsi_4h,
            oversold, overbought,
...
impl RsiStrategy {
    fn analyze_rsi_signals(
        &self,
        rsi_1h: f64, rsi_4h: f64,
        prev_rsi_1h: f64, prev_rsi_4h: f64,
