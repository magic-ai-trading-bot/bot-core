...
mod tests {
    ...
    async fn test_can_open_position_strong_buy_confidence_below_threshold() {
        ...
        assert!(
            !result,
            "Must reject StrongBuy with confidence 0.6999 (just below 0.7)"
        );
    }
    ...
    async fn test_can_open_position_strong_buy_confidence_at_exact_threshold() {
        ...
        assert!(
            result,
            "Must accept StrongBuy with confidence 0.7000 (exact threshold)"
        );
    }
    ...
    async fn test_can_open_position_strong_buy_confidence_above_threshold() {
        ...
        assert!(
            result,
            "Must accept StrongBuy with confidence 0.7001 (just above 0.7)"
        );
    }
    ...
    async fn test_can_open_position_buy_confidence_below_threshold() {
        ...
        assert!(
            !result,
            "Must reject Buy with confidence 0.7999 (just below 0.8)"
        );
    }
    ...
    async fn test_can_open_position_buy_confidence_at_exact_threshold() {
        ...
        assert!(
            result,
            "Must accept Buy with confidence 0.8000 (exact threshold)"
        );
    }
    ...
    async fn test_can_open_position_buy_confidence_above_threshold() {
        ...
        assert!(
            result,
            "Must accept Buy with confidence 0.8001 (just above 0.8)"
        );
    }
    ...
    async fn test_requires_both_good_confidence_and_good_risk_reward() {
        ...
        assert!(
            !result1,
            "Good confidence alone not enough (bad risk-reward should reject)"
        );
        ...
        assert!(
            !result2,
            "Good risk-reward alone not enough (bad confidence should reject)"
        );
        ...
        assert!(
            result3,
            "Both good confidence and risk-reward should accept"
        );
    }
    ...
    async fn test_hold_signal_always_rejected_despite_perfect_metrics() {
        ...
        assert!(
            !result,
            "Hold signal must be rejected regardless of perfect metrics"
        );
    }
    ...
    fn test_min_stop_loss_percentage_is_point_five_percent() {
        ...
        assert_eq!(
            size_below, config.default_quantity,
            "Below 0.5% stop loss must return default quantity"
        );

        // Stop loss at 0.5% (at threshold) → should calculate
        let size_at =
            rm.calculate_position_size("BTCUSDT", 50000.0, Some(50000.0 * (1.0 - 0.005)), 10000.0);
        assert!(
            size_at >= config.default_quantity * 0.01,
    ...
    async fn test_confidence_thresholds_are_point_seven_and_point_eight() {
        ...
        assert!(!rm
            .can_open_position("BTCUSDT", &strong_below)
            .await
            .unwrap());
    ...
    async fn test_risk_reward_threshold_is_one_point_five() {
        ...
        assert!(
            !rm.can_open_position("BTCUSDT", &below).await.unwrap(),
        ...
        assert!(
            rm.can_open_position("BTCUSDT", &at).await.unwrap(),
    ...
    async fn test_sell_signals_use_same_confidence_thresholds() {
        ...
        assert!(!rm
            .can_open_position("BTCUSDT", &strong_sell_below)
            .await
            .unwrap());
        let strong_sell_at = create_test_analysis(TradingSignal::StrongSell, 0.7);
        assert!(rm
            .can_open_position("BTCUSDT", &strong_sell_at)
            .await
            .unwrap());
