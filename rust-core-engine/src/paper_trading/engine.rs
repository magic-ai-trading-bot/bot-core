...
use super::{
    trade::{PaperTrade, TradeType, CloseReason},
    portfolio::PaperPortfolio,
    settings::PaperTradingSettings,
    strategy_optimizer::StrategyOptimizer,
    AITradingSignal, PaperTradingEvent, TradeExecutionResult,
    MarketAnalysisData, PerformanceSummary,
};
use uuid;
...
impl PaperTradingEngine {
    ...
    pub async fn new(
        ...
        let settings = match storage.load_paper_trading_settings().await {
            ...
            Err(e) => {
                warn!("⚠️ Failed to load settings from database, using defaults: {}", e);
                default_settings
            }
        };
        
        let portfolio = Arc::new(RwLock::new(PaperPortfolio::new(settings.basic.initial_balance)));
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        let (_price_result, _signal_result, _trade_result, _perf_result, _opt_result, _metrics_result) = tokio::try_join!(
            price_update_handle,
    ...
    fn start_daily_metrics_save(&self) -> tokio::task::JoinHandle<Result<()>> {
        ...
        tokio::spawn(async move {
            ...
            while *engine.is_running.read().await {
                ...
                if let Err(e) = engine.storage.save_daily_metrics(&portfolio, daily_pnl).await {
                    error!("Failed to save daily metrics: {}", e);
                } else {
                    info!("Saved daily metrics: PnL = {:.2}, Total Trades = {}", daily_pnl, portfolio.metrics.total_trades);
                }
        ...
        for symbol in symbols {
            match self.get_ai_signal_for_symbol(&symbol).await {
                Ok(signal) => {
                    ...
                    if let Err(e) = self.storage.save_ai_signal(&signal, executed, trade_id).await {
                            ...
                            Ok(result) => {
                                if result.success {
                                    ...
                                    if let Some(trade_id) = result.trade_id {
                                        ...
                                        info!("Trade executed for signal {}: {}", signal.id, trade_id);
                                    }
                                }
                            },
                            Err(e) => {
    ...
    async fn get_ai_signal_for_symbol(&self, symbol: &str) -> Result<AITradingSignal> {
        ...
        let candles: Vec<crate::market_data::cache::CandleData> = klines.into_iter().map(|kline| {
            crate::market_data::cache::CandleData {
        ...
        // Get AI analysis
        let ai_response = self.ai_service.analyze_for_trading_signal(&strategy_input, ai_request.strategy_context).await?;
    ...
    async fn process_trading_signal(&self, signal: AITradingSignal) -> Result<TradeExecutionResult> {
        ...
        let existing_positions = portfolio.get_open_trades()
            .iter()
    ...
    async fn execute_trade(&self, pending_trade: PendingTrade) -> Result<TradeExecutionResult> {
        ...
        info!("Executed paper trade: {} {} {} @ {} with {}x leverage", 
              trade_type.to_string(), pending_trade.calculated_quantity, 
              signal.symbol, signal.entry_price, pending_trade.calculated_leverage);
    ...
    pub async fn start_async(&self) -> Result<()> {
        ...
        tokio::spawn(async move {
            ...
            let (_price_result, _signal_result, _trade_result, _perf_result, _opt_result, _metrics_result) = tokio::join!(
                price_update_handle,
                signal_processing_handle,
                trade_monitoring_handle,
                performance_tracking_handle,
    ...
    pub async fn close_trade(&self, trade_id: &str) -> Result<()> {
        let current_price = {
            ...
            if let Some(trade) = portfolio.get_trade(trade_id) {
                self.current_prices.read().await.get(&trade.symbol).copied().unwrap_or(trade.entry_price)
            } else {
    ...
    pub async fn update_confidence_threshold(&self, threshold: f64) -> Result<()> {
        ...
        info!("✅ Confidence threshold updated to: {:.1}% and saved to database", threshold * 100.0);
        Ok(())
    }
