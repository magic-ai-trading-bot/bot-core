...
export const PARAMETER_BOUNDS: Record<string, ParameterBound> = {
  ...
  // ── GREEN: ATR / Regime filter parameters (auto-adjust + notify) ──
  // @spec:FR-RISK-010, FR-RISK-012 - ATR-based sizing and regime filters
  atr_stop_multiplier: {
    name: "ATR Stop Multiplier",
    tier: "GREEN",
    min: 0.8, max: 2.5, step: 0.1, type: "number",
    apiEndpoint: "/api/paper-trading/basic-settings",
    apiField: "atr_stop_multiplier",
    description: "ATR multiplier for stop loss distance (SL = entry ± ATR × multiplier). Lower = tighter stops.",
    defaultValue: 1.2,
    cooldownMs: SIX_HOURS,
  },
  atr_tp_multiplier: {
    ...
    tier: "GREEN",
    min: 1.5, max: 5.0, step: 0.1, type: "number",
    apiEndpoint: "/api/paper-trading/basic-settings",
    apiField: "atr_tp_multiplier",
    description: "ATR multiplier for take profit distance (TP = entry ∓ ATR × multiplier). Higher = wider targets.",
    defaultValue: 2.4,
    cooldownMs: SIX_HOURS,
  },
  funding_spike_threshold: {
    name: "Funding Spike Threshold",
    tier: "GREEN",
    min: 0.0001, max: 0.001, step: 0.0001, type: "number",
    apiEndpoint: "/api/paper-trading/basic-settings",
    apiField: "funding_spike_threshold",
    description: "Absolute funding rate threshold to trigger position size reduction (0.0003 = 0.03%)",
    defaultValue: 0.0003,
    cooldownMs: SIX_HOURS,
  },
  atr_spike_multiplier: {
    name: "ATR Spike Multiplier",
    tier: "GREEN",
    min: 1.5, max: 3.0, step: 0.1, type: "number",
    apiEndpoint: "/api/paper-trading/basic-settings",
    apiField: "atr_spike_multiplier",
    description: "ATR spike detection: current ATR > mean ATR × this multiplier triggers size reduction",
    defaultValue: 2.0,
    cooldownMs: SIX_HOURS,
  },
  consecutive_loss_reduction_pct: {
    name: "Consecutive Loss Reduction %",
    tier: "GREEN",
    min: 0.1, max: 0.5, step: 0.05, type: "number",
    apiEndpoint: "/api/paper-trading/basic-settings",
    apiField: "consecutive_loss_reduction_pct",
    description: "Per-loss reduction factor above threshold (0.3 = reduce 30% per additional consecutive loss)",
    defaultValue: 0.3,
    cooldownMs: SIX_HOURS,
  },

  // ── YELLOW: ATR / Kelly capital risk parameters (require confirmation) ──
  base_risk_pct: {
    name: "Base Risk % per Trade",
    tier: "YELLOW",
    min: 0.5, max: 5.0, step: 0.25, type: "number",
    apiEndpoint: "/api/paper-trading/basic-settings",
    apiField: "base_risk_pct",
    description: "Base risk percentage per trade for ATR sizing (size = equity × risk% / SL distance)",
    defaultValue: 2.0,
    cooldownMs: SIX_HOURS,
  },
  kelly_fraction: {
    name: "Kelly Fraction",
    tier: "YELLOW",
    min: 0.25, max: 0.75, step: 0.05, type: "number",
    apiEndpoint: "/api/paper-trading/basic-settings",
    apiField: "kelly_fraction",
    description: "Kelly criterion fraction (0.5 = Half-Kelly, safer than full Kelly). Lower = more conservative.",
    defaultValue: 0.5,
    cooldownMs: SIX_HOURS,
  },

  // ── RED: ATR / Kelly master switches and safety limits ──
  atr_stop_enabled: {
    name: "ATR Stop Loss Enabled",
    tier: "RED",
    type: "boolean",
    apiEndpoint: "/api/paper-trading/basic-settings",
    apiField: "atr_stop_enabled",
    description: "Enable ATR-based stop loss and position sizing. When disabled, uses existing PnL-based formula.",
    defaultValue: false,
    cooldownMs: ONE_HOUR,
  },
  kelly_enabled: {
    name: "Half-Kelly Enabled",
    tier: "RED",
    type: "boolean",
    apiEndpoint: "/api/paper-trading/basic-settings",
    apiField: "kelly_enabled",
    description: "Enable Half-Kelly criterion for dynamic risk scaling based on recent trade performance.",
    defaultValue: false,
    cooldownMs: ONE_HOUR,
  },
  weekly_drawdown_limit_pct: {
    ...
    apiEndpoint: "/api/paper-trading/basic-settings",
    apiField: "weekly_drawdown_limit_pct",
    description: "Maximum weekly portfolio drawdown before trading is paused until next week",
    defaultValue: 7.0,
    cooldownMs: SIX_HOURS,
  },
